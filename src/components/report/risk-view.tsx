            ...
            <div className="mx-auto max-w-5xl space-y-8">
                <header className="space-y-4">
                    ...
                    <p className="text-muted-foreground text-lg">
                        이 문서는 {data.title}에 대한 주요 리스크 및 대응 전략 분석 결과입니다.
                    </p>
                </header>
                                    ...
                                    <TableBody>
                                        {data.risk_matrix.map((item, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell className={cn("text-center", getSeverityClass(item.likelihood))}>
                                                    {item.likelihood}
                                                </TableCell>
                                                <TableCell className={cn("text-center", getSeverityClass(item.impact))}>
                                                    {item.impact}
                                                </TableCell>
